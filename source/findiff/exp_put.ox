...
/**
   This function calculates the put option price of an asset using the explicit finite difference method.
...
option_price_put_finite_diff_explicit(S,X,r,sigma,time,no_S_steps,no_t_steps, type) {
    ...
    for (m=0;m<=M;++m) {
	    f_next[m]=max(0.0,X-S_values[m]); // payoff for put option
	}
    ...
    for (decl t=N-1;t>=0;--t) {
		...
		for (m=1;m<M;++m) {
		    if (type == American){ // American option
		        ...
		        f[m] = max(f[m],X-S_values[m]);  // check if the asset is worth more exercised early
			}
